...
class OptionAnalyzer:
    ...
    def calculate_iv_and_greeks(self, model="BS", sample_iv_size=5, sample_points=1000):
        ...
        S = self.history.reset_index([1,2]).merge(self.underlying.history["Close"], left_index=True, right_index=True)
        ...
        while loop:
            try:
                call_results = self._calculate_iv_and_greeks_c_bs(loop_sample_iv_size, sample_points, hv, S, q, T_t, K, r)
            except ValueError:
        ...
        while loop:
            try:
                put_results = self._calculate_iv_and_greeks_p_bs(loop_sample_iv_size, sample_points, hv, S, q, T_t, K, r)
            except ValueError:
                loop_sample_iv_size *= 2
    ...
    def _calculate_iv_and_greeks_c_bs(self, sample_iv_size, sample_points, hv, S, q, T_t, K, r):
        ...
        if higher_than_max_vol.sum() > 0:
            raise ValueError("observed price exceeds max sample iv implied price. will try bigger sample iv size.")
        ...
        theta_c = ((-S * iv_c * np.exp(-q * T_t) * self._inverseNorm(d1)/ (2 * np.sqrt(T_t))) 
            -r * K * np.exp(-r * T_t) * norm.cdf(d2) 
            + q * S * np.exp(-q * T_t) * norm.cdf(d1)) /252
        
        rho_c = K * T_t * np.exp(-r * T_t) * norm.cdf(d2) / 100

        return (d1, d2, iv_c, c, delta_c, gamma_c, vega_c, theta_c, rho_c, idx_c, zero_vol_c, max_vol_c)

    def _calculate_iv_and_greeks_p_bs(self, sample_iv_size, sample_points, hv, S, q, T_t, K, r):
        """ calculate iv and greeks for call based on BS model.
        """
        ...
        theta_p = ((-S * iv_p * np.exp(-q * T_t) * self._inverseNorm(d1)/ (2 * np.sqrt(T_t))) 
            + r * K * np.exp(-r * T_t) * norm.cdf(d2) 
            - q * S * np.exp(-q * T_t) * norm.cdf(d1)) / 252
        
        rho_p = -K * T_t * np.exp(-r * T_t) * norm.cdf(-d2) / 100

        return (d1, d2, iv_p, p, delta_p, gamma_p, vega_p, theta_p, rho_p, idx_p, zero_vol_p, max_vol_p)
